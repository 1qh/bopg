...
const Delete = ({ id }: { id: Blog['_id'] }) => {
  ...
  Create = () => {
    const [open, setOpen] = useState(false),
      ...
      form = useForm({
        ...
        onSubmit: async d => create({ ...d, published: false, slug: slugify(d.title) }),
        onSuccess: () => {
          form.reset()
          setOpen(false)
        ...
        values: { category: 'tech', content: '', tags: [], title: '' }
      })
    return (
      <Dialog
        onOpenChange={v => {
        ...
        <DialogContent
          ...
          <Form
            ...
            form={form}
            render={({ Arr, Choose, Fields, Submit, Text }) => (
              <>
                <Fields>
  ...
  Author = ({ _id: id, author, category, className, own, published, tags, updatedAt }: Blog & ComponentProps<'div'>) => (
    <div className={cn('flex items-center', className)}>
      ...
      {own ? (
        <>
          <Publish className='mr-2' id={id} published={published} />
  ...
  Card = (props: Blog) => {
    const { _id: id, content, title } = props
    return (
  ...
  List = ({ blogs }: { blogs: Blog[] }) =>
    blogs.length ? (
      blogs.filter(b => b.published || b.own).map(b => <Card key={b._id} {...b} />)
    ) : (
      <p className='text-muted-foreground'>No posts yet</p>
    )

export { Author, Card, Create, List }
export type { Blog }
