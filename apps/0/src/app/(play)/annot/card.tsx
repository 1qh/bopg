...
type CardProps = RouterOutputs['annot']['all'][number] & {
  viewerId?: string
}
const Card = ({ ava, createdAt, description, id, tags, title, user, viewerId }: CardProps) => (
  <>
    {user.id === viewerId && (
      <Popover>
        ...
        <PopoverContent className='flex w-fit flex-col rounded-xl p-1 *:justify-start' side='right'>
          <Update {...{ ava, description, id, tags, title }} />
          <Delete id={id} />
        </PopoverContent>
      </Popover>
    )}
