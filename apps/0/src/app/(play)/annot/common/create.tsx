...
import { InsertAnnotSchema } from '@a/db/schema'
import { cn } from '@a/ui'
...
const Create = ({ buttonText, className, Icon = Plus }: CreateProps) => {
  const [open, setOpen] = useState(false),
    { annot } = api(),
    form = useForm({
      defaultValues: { description: '', tags: [] },
      resolver: standardSchemaResolver(InsertAnnotSchema)
    }),
    queryClient = useQueryClient(),
    { isPending, mutate } = useMutation(
      annot.insert.mutationOptions({
        onSuccess: async () => {
          setOpen(false)
          form.reset()
  ...
  return (
    <Dialog onOpenChange={setOpen} open={open}>
      ...
      <DialogContent className='gap-5'>
        <DialogTitle>New Annot</DialogTitle>
        <Form {...form}>
